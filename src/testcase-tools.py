...
def make_import_zip(args :argparse.Namespace):
    problem_name = args.zip
    verb_print(f"TASK: generate {problem_name}.zip", tcol.HEADER)

    zipf = zipfile.PyZipFile(f"{problem_name}.zip", "w")
    zipf.mkdir("data")
    ...
    if args.pdf is not None: zipf.write(args.pdf, "problem.pdf")
